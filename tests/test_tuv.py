...
class TestTuv(TestCase):
  ...
  def create_tuv_from_element(self):
    ...
    for attr in tuv.__slots__:
      ...
      match attr:
        ...
        case "lang":
          self.assertEqual(val, elem.get("{http://www.w3.org/XML/1998/namespace}lang"))
        case _:
  ...
  def test_export_full_tuv(self):
    tuv = Tuv(
      lang="en-US",
      segment=[
        "text of the first seg",
        Bpt(content="text of the bpt", i=1),
        "tail of the bpt",
        Ept(content="text of the ept", i=1),
        "tail of the ept",
        Ph(content="text of the ph", i=1),
        "tail of the ph",
      ],
      encoding="utf-8",
      datatype="PlainText",
      usagecount=1,
      lastusagedate=datetime.now(UTC),
      creationtool="PythonTmx Test Suite",
      creationtoolversion="0.3",
      creationdate=datetime.now(UTC),
      creationid="PythonTmx",
      changedate=datetime.now(UTC),
      changeid="Testing",
      tmf="testing",
      notes=(Note(text=str(x)) for x in range(10)),
      props=(Prop(text=str(x), type="x-test") for x in range(10)),
    )
    elem = tuv.to_element()
    self.assertEqual(elem.tag, "tuv")
    self.assertEqual(len(elem), 21)
    self.assertEqual(len(elem.attrib), 12)
    self.assertEqual(elem.get("{http://www.w3.org/XML/1998/namespace}lang"), "en-US")
    self.assertEqual(elem.find("seg").text, "text of the first seg")
    self.assertEqual(elem.find("seg").find("bpt").text, "text of the bpt")
    self.assertEqual(elem.find("seg").find("bpt").get("i"), "1")
    self.assertEqual(elem.find("seg").find("bpt").tail, "tail of the bpt")
    self.assertEqual(elem.find("seg").find("ept").text, "text of the ept")
    self.assertEqual(elem.find("seg").find("ept").get("i"), "1")
    self.assertEqual(elem.find("seg").find("ept").tail, "tail of the ept")
    self.assertEqual(elem.find("seg").find("ph").text, "text of the ph")
    self.assertEqual(elem.find("seg").find("ph").get("i"), "1")
    self.assertEqual(elem.find("seg").find("ph").tail, "tail of the ph")
    self.assertEqual(elem.get("o-encoding"), "utf-8")
    self.assertEqual(elem.get("datatype"), "PlainText")
    self.assertEqual(elem.get("usagecount"), "1")
    self.assertEqual(
      elem.get("lastusagedate"), tuv.lastusagedate.strftime(r"%Y%m%dT%H%M%SZ")
    )
    self.assertEqual(elem.get("creationtool"), "PythonTmx Test Suite")
    self.assertEqual(elem.get("creationtoolversion"), "0.3")
    self.assertEqual(
      elem.get("creationdate"), tuv.creationdate.strftime(r"%Y%m%dT%H%M%SZ")
    )
    self.assertEqual(elem.get("creationid"), "PythonTmx")
    self.assertEqual(elem.get("changedate"), tuv.changedate.strftime(r"%Y%m%dT%H%M%SZ"))
    self.assertEqual(elem.get("changeid"), "Testing")
    self.assertEqual(elem.get("o-tmf"), "testing")
    self.assertEqual(len(elem.findall("note")), 10)
    self.assertEqual(len(elem.findall("prop")), 10)
    for id, item in enumerate(elem.findall("note")):
      self.assertEqual(item.text, str(id))
      self.assertEqual(item.tag, "note")
    for id, item in enumerate(elem.findall("prop")):
      self.assertEqual(item.text, str(id))
      self.assertEqual(item.get("type"), "x-test")
      self.assertEqual(item.tag, "prop")
