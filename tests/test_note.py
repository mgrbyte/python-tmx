...
class TestNote(TestCase):
  ...
  def test_create_note_from_element(self):
    elem = fromstring("""<note xml:lang="en-US" o-encoding="utf-8" >note text</note>""")
    note = Note(elem)
    self.assertEqual(note._source_elem, elem)
    self.assertEqual(note.text, elem.text)
    self.assertEqual(note.lang, elem.get("{http://www.w3.org/XML/1998/namespace}lang"))
  ...
  def test_create_note_from_element_with_kwargs(self):
    note = Note(
      fromstring("""<note xml:lang="en-US" o-encoding="utf-8">note text</note>"""),
      text="override text",
    )
  ...
  def test_export_full_note(self):
    note = Note(
      text="note text",
      lang="en-US",
      encoding="utf-8",
