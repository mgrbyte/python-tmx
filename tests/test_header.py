...
class TestHeader(TestCase):
  ...
  def test_export_full_header(self):
    header = Header(
      creationtool="creationtool",
      creationtoolversion="1.0",
      segtype="sentence",
      ...
      udes=(Ude(name=str(x)) for x in range(10)),
    )
    elem = header.to_element()
    self.assertEqual(elem.tag, "header")
    self.assertEqual(len(elem), 30)
    self.assertEqual(len(elem.attrib), 12)
    self.assertEqual(elem.get("creationtool"), "creationtool")
    self.assertEqual(elem.get("creationtoolversion"), "1.0")
    self.assertEqual(elem.get("segtype"), "sentence")
    self.assertEqual(elem.get("o-tmf"), "tmf")
    self.assertEqual(elem.get("adminlang"), "en-us")
    ...
    self.assertEqual(
      elem.get("creationdate"), header.creationdate.strftime(r"%Y%m%dT%H%M%SZ")
    )
    self.assertEqual(elem.get("creationid"), "Enzo")
    self.assertEqual(
      elem.get("changedate"), header.changedate.strftime(r"%Y%m%dT%H%M%SZ")
    )
    self.assertEqual(elem.get("changeid"), "DJ Khaled")
    self.assertEqual(len(elem.findall("note")), 10)
    self.assertEqual(len(elem.findall("prop")), 10)
    self.assertEqual(len(elem.findall("ude")), 10)
    ...
    for id, item in enumerate(elem.findall("ude")):
      self.assertEqual(item.get("name"), str(id))
      self.assertEqual(item.tag, "ude")
