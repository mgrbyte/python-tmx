...
class TestTu(TestCase):
  ...
  def test_export_full_tu(self):
    tu = Tu(
      ...
      tmf="testing",
      srclang="en-us",
      tuvs=list(Tuv(lang="en-US", segment=str(x)) for x in range(10)),
      notes=(Note(text=str(x)) for x in range(10)),
      props=(Prop(text=str(x), type="x-test") for x in range(10)),
    )
    elem = tu.to_element()
    self.assertEqual(elem.tag, "tu")
    self.assertEqual(len(elem), 30)
    self.assertEqual(len(elem.attrib), 14)
    self.assertEqual(elem.get("tuid"), "coolid")
    self.assertEqual(elem.get("o-encoding"), "utf-8")
    self.assertEqual(elem.get("datatype"), "Plaintext")
    self.assertEqual(elem.get("usagecount"), "12")
    self.assertEqual(
      elem.get("lastusagedate"), tu.lastusagedate.strftime(r"%Y%m%dT%H%M%SZ")
    )
    self.assertEqual(elem.get("creationtool"), "PythonTmx Test Suite")
    self.assertEqual(elem.get("creationtoolversion"), "0.3")
    self.assertEqual(
      elem.get("creationdate"), tu.creationdate.strftime(r"%Y%m%dT%H%M%SZ")
    )
    self.assertEqual(elem.get("creationid"), "PythonTmx")
    self.assertEqual(elem.get("changedate"), tu.changedate.strftime(r"%Y%m%dT%H%M%SZ"))
    self.assertEqual(elem.get("segtype"), "sentence")
    self.assertEqual(elem.get("changeid"), "Testing")
    self.assertEqual(elem.get("o-tmf"), "testing")
    self.assertEqual(elem.get("srclang"), "en-us")
    self.assertEqual(len(elem.findall("tuv")), 10)
    self.assertEqual(len(elem.findall("note")), 10)
    self.assertEqual(len(elem.findall("prop")), 10)
    for id, item in enumerate(elem.findall("tuv")):
      self.assertEqual(item.find("seg").text, str(id))
      self.assertEqual(item.get("{http://www.w3.org/XML/1998/namespace}lang"), "en-US")
      self.assertEqual(item.tag, "tuv")
