...
XMLLANG_NAMESPACE_ATTR: str = "{http://www.w3.org/XML/1998/namespace}lang"
...
def load_ept(ept_elem: _Element) -> tuple[ept, run]:
...
def load_hi(hi_elem: _Element) -> tuple[hi, run]:
    hi_run: hi = hi()
    tail_run: run = run()
...
def load_it(it_elem: _Element) -> tuple[it, run]:
    it_run: it = it()
    tail_run: run = run()
...
def load_tuv(tuv_elem: _Element) -> tuv:
    ...
    seg_elem: _Element | None = tuv_elem.find("seg")
...
def load_tmx(file: Path | str) -> tmx:
    ...
    tmx_header_elem: _Element | None = tmx_root.find("header")
